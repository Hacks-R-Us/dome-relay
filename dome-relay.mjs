...
for (const [i, segment] of config.segments.entries()) {
    ...
    for (const path of segment.paths) {
        const leds = path.struts.reduce((prev, curr) => prev + curr.leds.length, 0);
        paths.push({
            startIndex: numLeds,
            numLeds: leds
        });
...
for (const [i, segment] of segments.entries()) {
    console.log(`Segment ${i} | Leds: ${segment.numLeds} | Paths: ${segment.paths.map((p) => `${p.startIndex}-${p.startIndex + p.numLeds}-(${p.numLeds})`).join(',')}`);
}
console.log();
...
udpServer.on('message', (msg, _remote) => {
    ...
    if (!segmentConfig)
        return;
    if (packet.length !== segmentConfig.numLeds * 3)
        return;
